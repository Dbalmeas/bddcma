  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        // CMA CGM colors: Bleu foncé (#002D59) et Rouge/Orange (#FF6F00)
        // Utiliser la position pour créer une variation de couleur avec plus de rouge visible
        float colorMix = sin(vInitialPosition.x * 2.0 + vInitialPosition.y * 3.0 + uTime * 0.5) * 0.5 + 0.5;
        
        // Bleu CMA CGM: rgb(0, 45, 140) = vec3(0.0, 0.176, 0.549)
        vec3 blueColor = vec3(0.0, 0.176, 0.549) * 1.8;
        // Rouge CMA CGM officiel: rgb(239, 64, 53) = vec3(0.937, 0.251, 0.208)
        vec3 redColor = vec3(0.937, 0.251, 0.208) * 1.5;
        
        // Mixer les couleurs avec plus de rouge visible (60% bleu, 40% rouge, avec accent sur le rouge)
        // Utiliser une fonction pour créer des zones plus rouges
        float redIntensity = pow(colorMix, 0.7); // Augmente la visibilité du rouge
        vec3 baseColor = mix(blueColor, redColor, redIntensity * 0.5);
