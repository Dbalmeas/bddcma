/**
 * Script d'ingestion ÉCHANTILLONNÉ des données JSONL vers Supabase
 * Version avec échantillonnage: 200 événements par fichier maximum
 * Projet Everdian x Albert School
 *
 * Usage:
 *   npm run ingest:sample -- <chemin-vers-fichier.jsonl>
 *   npm run ingest:sample:all  (pour tous les fichiers avec échantillonnage)
 */
...
// Configuration
const BATCH_SIZE = 100 // Nombre d'événements par batch (réduit pour échantillonnage)
...
/**
 * Traite un fichier JSONL avec échantillonnage
 */
async function processFile(filePath: string): Promise<void> {
  stats.currentFile = path.basename(filePath)
  stats.totalFiles++
  console.log(`\n📂 [${stats.totalFiles}] Traitement de ${stats.currentFile}...`)
  console.log(`   📊 Limite: ${MAX_EVENTS_PER_FILE} événements par fichier`)
  ...
  for await (const line of rl) {
    ...
    // 🎯 ARRÊTER si on a atteint la limite pour ce fichier
    if (eventsFromThisFile >= MAX_EVENTS_PER_FILE) {
      console.log(`   ⏸️  Limite atteinte (${MAX_EVENTS_PER_FILE} événements)`)
      break
    }
  ...
  console.log(`  ✅ ${stats.currentFile} terminé (${eventsFromThisFile} événements importés sur ${lineCount} lignes)`)
}
 ...
 */
async function main() {
  console.log('🚀 Démarrage de l\'ingestion ÉCHANTILLONNÉE des données...\n')
  ...
  if (args.length === 0) {
    console.error('❌ Usage: npm run ingest:sample -- <fichier.jsonl>')
    console.error('   ou: npm run ingest:sample:all')
    process.exit(1)
  }
